...
HOST = "127.0.0.1"
PARTITION_DIR = "bisleri_partitions"

# Regex to validate Bisleri logs
...
def get_partition_file(service, level):
    ...
    with partitions_master_lock:
        if key not in partition_files:
            ...
            print(f"[Storage] Created new partition file: {filename}")
...
def harvest_from_branch(branch_name, port):
    ...
    print(f"[{branch_name}] Connected and harvesting on port {port}")
...
def print_stats_periodically():
    while True:
        ...
        with stats_lock:
            if not stats:
                continue
            print("\n--- Live Bisleri Data Ingestion Stats ---")
            for (branch, level), count in sorted(stats.items()):
                print(f"  {branch:18s} {level:10s} {count} logs")
            print("-----------------------------------------\n")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\nShutting down harvester. Saving files...")
